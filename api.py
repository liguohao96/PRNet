...
class PRN:
    ...
    Args:
        ...
        model(str, optional): name of weight file
    '''

    def __init__(self, is_dlib=False, prefix='.', model=None):
        ...
        if is_dlib:
            ...
            detector_path = os.path.join(
                prefix, 'net-data/mmod_human_face_detector.dat')
        ...
        model = '256_256_resfcn256_weight' if model is None else model
        prn_path = os.path.join(prefix, 'net-data', model)
        ...
        self.uv_kpt_ind = np.loadtxt(
            prefix + '/uv-data/uv_kpt_ind.txt').astype(np.int32)  # 2 x 68 get kpt
        # get valid vertices in the pos map
        self.face_ind = np.loadtxt(
            prefix + '/uv-data/face_ind.txt').astype(np.int32)
        self.triangles = np.loadtxt(
            prefix + '/uv-data/triangles.txt').astype(np.int32)  # ntri x 3
    ...
    def process(self, input, image_info=None):
        ...
        imsave('cropped.jpg', cropped_image)
    ...
    def get_vertices(self, pos):
        ...
        print('all vertices')
        print(np.shape(all_vertices))
        print(np.min(all_vertices, axis=(0)), np.max(
            all_vertices, axis=(0)), np.mean(all_vertices, axis=(0)))
        print(self.face_ind)
        vertices = all_vertices[self.face_ind, :]
        # vertices = all_vertices
        print('vertices')
        print(np.shape(vertices))
        print(np.min(vertices, axis=(0)), np.max(
            vertices, axis=(0)), np.mean(vertices, axis=(0)))
